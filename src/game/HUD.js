...
export class HUD {
  ...
  showResults({ place, time, standings }) {
    if (!this.results) return;
    const places = ['🥇 1-е место!', '🥈 2-е место!', '🥉 3-е место!'];
    const title = document.getElementById('results-title');
    if (title) title.textContent = places[place - 1] || 'Финиш!';
    const timeEl = document.getElementById('results-time');
    if (timeEl) timeEl.textContent = `Время: ${formatTime(time)}`;
    if (this.resultsBody && standings) {
      this.resultsBody.innerHTML = standings
        .map(
          (s) =>
            `<div class="result-row"><span class="place">${s.place}.</span> <span class="name">${s.name}</span> <span class="lap">круг ${s.lap}</span></div>`
        )
        .join('');
    }
